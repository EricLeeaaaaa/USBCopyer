...
- **增量同步**: 对同一个U盘设备，程序只会复制新增或修改过的文件，避免重复备份，节省空间。
...
2.  **插入USB磁盘**。程序会自动检测到新插入的USB磁盘，并根据配置文件开始同步文件。
...
# 目标文件保存目录，可用占位符: <drivelabel> - 驱动器卷标, <volumeserial> - 卷序列号
...
- `SavePath`:  设置文件保存的目标目录。可以使用以下占位符：
    - `<drivelabel>`:  USB磁盘的卷标。
    - `<volumeserial>`: USB磁盘的卷序列号，用于唯一标识设备。
    - `<date>`:  当前日期，格式为 `YYYY-MM-DD`。
    - `<time>`:  当前时间，格式为 `HH-MM-SS`。
    例如，`./.saved/<drivelabel>_<volumeserial>/` 表示为每个U盘创建一个唯一的、固定的备份目录。

## 保存目录和日志

- 默认情况下，文件会保存在 `./.saved/<drivelabel>_<volumeserial>/` 目录下，`.saved` 目录是隐藏的。您可以通过在资源管理器中直接输入地址来访问此目录，或者在配置文件中修改目标保存路径。
- 程序日志也会输出到 `.saved` 目录下。
